...
- 分析当前笔记时会同步扫描 H1-H3 标题，并结合正文推荐更合适标题。
- 支持检查含英文或英文符号的标题，要求英文单词使用反引号包裹。
...
- 当前笔记内容。
- 用户填写的补充说明。
- 标题审查会随单文件分析一起执行，发送当前文件正文、H1-H3 标题、路径和行号。

AI 返回结果会被解析为结构化数据，用于渲染推荐卡片和执行操作。

归档推荐和标题审查在同一次 Chat Completions 兼容请求中完成。
...
6. 根据需要执行移动、重命名或复制路径。
7. 在同一次分析结果中查看标题建议，并点击「应用建议」或「应用修正」直接修改标题。
...
src/
  ...
  styles.css               插件样式
  title-audit.ts           当前文件标题扫描与 AI 审查
